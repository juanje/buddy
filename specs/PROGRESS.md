...
| `build-worker` Windows target + NSIS (Block 2) | **in progress** | `build-worker.ts` + Windows `.exe` green locally; `nsis` + `icon.ico` in conf; full `tauri build` needs Rust toolchain on host. |
