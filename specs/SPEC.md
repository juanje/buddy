...
subdirectory and the generated `glossary.md` are named there like
...
**FR-TASKM-10 — Parking, visibility, and staleness prompts**

- `agents-base.md` includes a "Parking and visibility" subsection with `@someday` guidance, date-gating note, and hidden-count surfacing.
- `consolidation.md` weekly depth includes a staleness review step (`staleDays` > 30, propose keep/complete/park/remove).
- `consolidation.md` daily step 4 mentions `parkedCount`/`futureCount` in Day summary.
...
| NFR-PORT-05 | Core app prompts live in `~/.buddy/prompts/`, not inside rootDir. On any app semver change, bundled prompts overwrite `~/.buddy/prompts/`; self-docs (`~/.buddy/docs/`) are deployed on every boot (see NFR-MIGRATE-06). User content in rootDir is never touched. |
...
| NFR-MIGRATE-06 | On boot, compare app semver (from `package.json` / Tauri version) with `last_app_version` in `~/.buddy/config.json`. If `config.json` is absent, `last_app_version` is absent, or semver differs: deploy bundled prompts and templates to `~/.buddy/`, then set `last_app_version` to the current semver. Self-documentation (`~/.buddy/docs/`) is deployed unconditionally on every boot (idempotent, after the RPC channel is up) — this prevents stale docs when a previous boot wrote `last_app_version` but crashed before doc deployment reached. No separate version file. |
