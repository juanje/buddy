...
| NFR-CONFIG-05 | One resolver for the global config directory: `globalConfigDir()`. It honours `BUDDY_CONFIG_DIR` first, then the directory of `BUDDY_CONFIG_PATH` (or its legacy alias), then `~/.buddy`. `defaultConfigDir()` delegates to it rather than deriving its own answer. This matters because the two are consulted by different processes: the worker resolved `usage.json` one way while the reflect child, a separate process (FR-REFLECT-06), resolved it the other, so a run could bill against a file nobody was reading and the budget cap would undercount. |
