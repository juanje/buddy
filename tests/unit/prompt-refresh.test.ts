// tests/unit/prompt-refresh.test.ts — NFR-MIGRATE-06 prompt refresh.
...
describe("prompt refresh", () => {
  ...
  it("refreshes prompts when app version differs", () => {
    configDir = mkdtempSync(join(tmpdir(), "ab-prompt-refresh-"));
    ...
    const refreshed = refreshPromptsIfNeeded(configDir, "0.2.0");
  ...
  it("creates config.json on fresh install", () => {
    ...
    const refreshed = refreshPromptsIfNeeded(configDir, "0.1.0");
