...
When(
  ...
  function (this: AbWorld, op: string, path: string) {
...
When("the user allows the permission", async function (this: AbWorld) {
  await this.controller.respondPermission(this.lastPermissionId!, true);
});

When("the user denies the permission", async function (this: AbWorld) {
...
Then(
  ...
  function (this: AbWorld, op: string) {
...
Then("the card offers allow-once and deny actions", function (this: AbWorld) {
  // Unresolved card = actions available; the view renders buttons for it.
  const cards = get(this.controller.permissions);
  assert.equal(cards[0].verdict, undefined);
});

Then("the worker receives an allow verdict for that request", function (this: AbWorld) {
  assert.deepEqual(this.permissionResolutions, [{ id: this.lastPermissionId, allow: true }]);
});

Then("the worker receives a deny verdict for that request", function (this: AbWorld) {
  assert.deepEqual(this.permissionResolutions, [{ id: this.lastPermissionId, allow: false }]);
});

Then("the card is marked as allowed", function (this: AbWorld) {
  assert.equal(get(this.controller.permissions)[0].verdict, "allowed");
});

Then("the card is marked as denied", function (this: AbWorld) {
  assert.equal(get(this.controller.permissions)[0].verdict, "denied");
});

Then("the permission card does not block the chat input", function (this: AbWorld) {
